...
class StargateConfig:
    ...
    def get_visible_configs(self):
        ...
        visible_hardware_keys = set()
        try:
            visible_hardware_keys = set(self.get("hardware_config_fields"))
        ...
        for key, config in self.config.items():
            if config.get("hidden"):
                continue
            if key in hardware_keys and key not in visible_hardware_keys:
                continue
            visible[key] = config
        return visible
    ...
    def set_bulk(self, data):
        ...
        '''
        if "stepper_use_motor_hat" in data:
            try:
                use_motor_hat = self.is_valid_value("stepper_use_motor_hat", data["stepper_use_motor_hat"])
            except ValueUnchanged:
                use_motor_hat = self.get("stepper_use_motor_hat")
            data["stepper_driver"] = "motorkit" if use_motor_hat else "tmc2209"
    ...
    def is_valid_value( self, key, test_value ):
        ...
        if required_type == "bool":
        ...
        elif required_type in ("str-enum", "string-enum"):
            if not isinstance(test_value, str ):
                raise ValueError(f"{key} must be type `str`")
            ...
            if param_config['max_value'] is not False and test_value > param_config['max_value']:
                raise ValueError(f"{key} Maximum value: {param_config['max_value']}")
            if param_config['min_value'] is not False and test_value < param_config['min_value']:
        ...
        expected_type = type_aliases.get(base_type, base_type)
        test_type = test_value.__class__.__name__
        if not nullable and test_type != expected_type:
            raise ValueError(f"Validation yielded invalid type for field {key}. Requires {expected_type}, yielded {test_type}")
    ...
    def save(self, sort=False):
        ...
        target_path = self.get_full_file_path()
        temp_path = target_path + ".tmp"

        try:
            file_mode = os.stat(target_path).st_mode & 0o777
        except FileNotFoundError:
            file_mode = 0o644
        ...
        dir_fd = os.open(os.path.dirname(target_path), os.O_DIRECTORY)
        try:
            os.fsync(dir_fd)
        finally:
            os.close(dir_fd)
