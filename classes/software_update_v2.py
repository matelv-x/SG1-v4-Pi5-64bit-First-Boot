...
class SoftwareUpdateV2:
    ...
    def __init__(self, app):
        ...
        # Initialize the GitPython repo object. Pi 5 recovery packages may be
        # installed from a ZIP instead of a real git checkout.
        try:
            self.repo = git.Repo('.')
    ...
    def get_available_updates(self):
        ...
        if not self.repo:
            self.log.log("Software updates disabled: not installed from git")
            return {}
        ...
        if not hasattr(self.repo.remotes, "origin"):
            self.log.log("Software updates disabled: git remote origin is not configured")
            return {}
