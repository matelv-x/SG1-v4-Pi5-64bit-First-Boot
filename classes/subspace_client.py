...
class SubspaceClient:
    ...
    def get_public_key(self):
        ...
        try:
            ...
            self._public_key_cache = subprocess.check_output(cmd, shell=True).decode('ascii')
    ...
    def is_online(self):
        now = time.monotonic()
        if self._online_cache is not None and now - self._online_checked_at < 30:
            return self._online_cache
        self._online_cache = self.net_tools.ping(self.keep_alive_address)
