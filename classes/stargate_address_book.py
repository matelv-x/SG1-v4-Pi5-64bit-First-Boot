...
class StargateAddressBook:
    ...
    def get_fan_gates(self):
        ...
        if local_address and not self.cfg.get("subspace_ip_address"):
            for gate_name, gate in list(gates.items()):
                if gate.get("gate_address") == local_address:
    ...
    def set_fan_gate(self, name, gate_address, ip_address, is_gate_online, is_black_hole=False):
        ...
        # TODO: Ensure unique address
        if self.get_local_address() == gate_address and not self.cfg.get("subspace_ip_address"):
            return
        fan_gates = self.get_fan_gates()
