...
class NetworkTools:
    ...
    def __init__(self, log):
        ...
        self.log = log
        self._internet_access_cache = None
        self._internet_access_checked_at = 0
    ...
    def has_internet_access(self): # was called check_internet_connection
        ...
        now = time.monotonic()
        if self._internet_access_cache is not None and now - self._internet_access_checked_at < 30:
            return self._internet_access_cache

        cloudflare_check = self.check_net(self.cloudflare)
        if 'open' in cloudflare_check or 'succeeded' in cloudflare_check:
            self._internet_access_cache = True
            self._internet_access_checked_at = now
            return True
        ...
        self._internet_access_cache = False
        self._internet_access_checked_at = now
        return False
