  ...
  $.ajax({
      ...
      success: function (response) {
        ...
        $.each(response, function (paramName, item) {
            ...
            paramPrettyName = item.display_name || toProperCase(paramName)
            group = item.config_group || getParamGroupByPrettyName(paramPrettyName)
...
function getHTMLFloat( paramName, data ){
  if ( data.protected ) return data.value
  step = data.step_value || "any"
  return "<input type='number' name='" + paramName + "' value='" + data.value + "' step='" + step + "' min='" + data.min_value + "' max='" + data.max_value + "'>"
}
