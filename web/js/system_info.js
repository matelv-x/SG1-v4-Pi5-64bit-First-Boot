  ...
  $.get('stargate/get/system_info')
    .done(function(data) {
      ...
      if (data.local_stargate_address_string){
        ...
        html = ""
        addressLength = getLocalAddressLength(data)
      ...
      $( "#config-lcl-sg-addr" ).button().on( "click", function() {
        $( "#dialog-form" ).dialog( "open" );
          ...
          for (i=1;i<=9;i++){
            ...
            if (i <= addressLength) {
              field.closest('.local-symbol-row').show()
            }
            else {
              field.closest('.local-symbol-row').hide()
            }
          }
          for (i=0;i<addressLength;i++){
...
function getLocalAddressLength(data){
  if (data.local_stargate_address && data.local_stargate_address.length >= 6){
    return Math.min(data.local_stargate_address.length, 9)
  }
  return 6
}
